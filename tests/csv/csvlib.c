...
void appendCsvRow(CSVFile *csv, const char *newRow) {
    // Make a copy of newRow so we don't modify the original string
    ...
    if (token_count != csv->headings_count) {
        ...
        free(newRowCopy);  // Don't forget to free the allocated memory
        return;
    }
    ...
    free(newRowCopy);  // Free the allocated memory once done
}
